type ProcessPayload = { body: unknown; contentType?: string };
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const upstream = await fetch('https://sh.dataspace.copernicus.eu/process/v1', {
      method: 'POST',
      headers: { authorization: `Bearer ${token.access_token}`, 'content-type': payload.contentType || 'application/json' },
      body: JSON.stringify(payload.body),
    });
    const data = await upstream.arrayBuffer();
    return new Response(data, { status: upstream.status, headers: { 'content-type': upstream.headers.get('content-type') || 'application/octet-stream', 'cache-control': 'no-store' } });
  } catch (error) {
    return new Response(JSON.stringify({ error: error instanceof Error ? error.message : 'Processing request failed' }), { status: 400, headers: { 'content-type': 'application/json' } });
  }
}
