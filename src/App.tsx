...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-200 flex">
    ...
    <main className="flex-1 overflow-y-auto"><header className="sticky top-0 z-30 bg-slate-950/90 backdrop-blur border-b border-slate-800 px-6 py-3"><div className="flex items-center justify-between"><div><h1 className="text-base font-bold text-white">{view === 'overview' && 'Executive Overview Dashboard'}{view === 'governance' && 'MPLADS Governance Intelligence Center'}{view === 'map' && 'Ward-Wise Geospatial Risk View'}{view === 'deepdive' && 'Multi-Modal Fraud Deep-Dive'}</h1><p className="text-xs text-slate-500">KAVACH · MPLADS AI-Driven Audit & Fraud Forensics Engine · Bangalore demonstration jurisdiction</p></div><div className="flex items-center gap-2 text-xs text-slate-500"><Activity className="w-3.5 h-3.5 text-emerald-400" />Live demo engine <span className="text-slate-700">|</span><Zap className="w-3.5 h-3.5 text-amber-400" />Deterministic analytics</div></div></header>
