...
import { ShieldCheck, LayoutDashboard, Map, FileSearch, AlertTriangle, Landmark, UserRound, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export function Sidebar({ activeView, onNavigate, alertCount, onProfile, user }: SidebarProps) {
  ...
  return <aside className={`${collapsed ? 'md:w-[76px]' : 'md:w-64'} w-full md:h-screen md:sticky md:top-0 shrink-0 border-b md:border-b-0 md:border-r border-slate-800 bg-slate-950 flex flex-col transition-all duration-200 z-40`}>
    <div className={`${collapsed ? 'md:p-3' : 'p-4 md:p-5'} border-b border-slate-800`}>
      <div className="flex items-center justify-between gap-2">
        <button onClick={()=>setCollapsed(v=>!v)} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} className="flex items-center gap-3 min-w-0">
        ...
        <button onClick={()=>setCollapsed(v=>!v)} className="hidden md:flex w-8 h-8 rounded-lg border border-slate-800 items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800" title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{collapsed ? <PanelLeftOpen className="w-4 h-4"/> : <PanelLeftClose className="w-4 h-4"/>}</button>
      </div>
    </div>
